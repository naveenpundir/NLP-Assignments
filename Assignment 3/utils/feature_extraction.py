...
class DataParams:  # data, embedding, model path etc.
    # Data Paths
    data_dir_path = "./data"
    train_path = "en-ud-train.conllu"
    valid_path = "en-ud-dev.conllu"
    ...
    def get_transition_from_current_state(self):  # logic to get next transition
        ...
        if stack_token_1.token_id >= 0 and stack_token_1.head_id == stack_token_0.token_id:  # left arc
            return 0
                ...
                and stack_token_0.token_id not in [x.head_id for x in self.buff]:
            return 1  # right arc
        else:
...
class Dataset(object):
    ...
    def build_embedding_matrix(self):
        ...
        # load word vectors
        word_vectors = {}
        embedding_lines = open(os.path.join(DataParams.data_dir_path, DataParams.embedding_file), "r").readlines()
        ...
        word_embedding_matrix = np.asarray(
            ...
            dtype=np.float32)
        for (word, idx) in list(self.word2idx.items()):
...
class FeatureExtractor(object):
    ...
    def extract_for_current_state(self, sentence, word2idx, pos2idx, dep2idx):
        ...
        # pos features -> 18
        pos_features.extend([x.pos for x in direct_tokens])
        pos_features.extend([x.pos for x in children_tokens])
    ...
    def create_instances_for_data(self, data, word2idx, pos2idx, dep2idx):
        ...
        dep_inputs = np.array(dep_inputs)
...
class DataReader(object):
    ...
    def read_data(self, data_lines):
        ...
        for token_conll in data_lines:
            ...
            if token_conll.startswith('#'):
                continue
            if len(token_conll) > 0:
                if "." in token_conll.split("\t")[0]:
                    continue
...
def load_datasets(load_existing_dump=False):
    ...
    train_lines = open(os.path.join(DataParams.data_dir_path, DataParams.train_path), "r").readlines()
    valid_lines = open(os.path.join(DataParams.data_dir_path, DataParams.valid_path), "r").readlines()
    test_lines = open(os.path.join(DataParams.data_dir_path, DataParams.test_path), "r").readlines()
    ...
    dataset.build_vocab()
    dump_pickle(dataset.word2idx, os.path.join(DataParams.dump_dir, DataParams.word_vocab_file))
    dump_pickle(dataset.pos2idx, os.path.join(DataParams.dump_dir, DataParams.pos_vocab_file))
    dump_pickle(dataset.dep2idx, os.path.join(DataParams.dump_dir, DataParams.dep_vocab_file))
    ...
    dump_pickle(dataset.word_embedding_matrix, os.path.join(DataParams.dump_dir, DataParams.word_emb_file))
    dump_pickle(dataset.pos_embedding_matrix, os.path.join(DataParams.dump_dir, DataParams.pos_emb_file))
    dump_pickle(dataset.dep_embedding_matrix, os.path.join(DataParams.dump_dir, DataParams.dep_emb_file))
